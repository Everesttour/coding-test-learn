...
정의: HashMap에서 key 값을 가져오고, 없으면 defaultValue를 반환

HashMap.containsKey(key)
정의: HashMap에 특정 key가 존재하는지 확인

HashMap.keySet()
정의: HashMap의 모든 key를 Set 형태로 반환

List.sort(Comparator)
정의: List를 비교 기준(Comparator)에 따라 정렬

Integer.compare(x, y)
정의: Integer 클래스의 정수 비교 함수
